...
- **Key Features**:
    ...
    - **Simple Settings (সহজ সেটিংস)**: Manages institution branding and academic year only. Marhala management has been moved to the Academic module (একাডেমিক) for centralized academic hierarchy control.
    ...
    - **AcademicHierarchySelector Component**: Reusable component for semester-centric filtering across all major modules. Uses `onSelectionChange` callback returning `{ marhala_id, department_id, semester_id, marhala_name, department_name, semester_name }`. Supports `showAllOption` and `layout` props. Integrated across: StudentList, StudentIDCard, Certificates (Transfer), Reports, Fee Setup, AdmissionFees, Fee Collection, AcademicCMS (Books/Papers), QuizTool, AISummary, and AINotes. All components store hierarchy IDs for backend queries alongside joined display names (format: "মারহালা | বিভাগ | সেমিস্টার").
